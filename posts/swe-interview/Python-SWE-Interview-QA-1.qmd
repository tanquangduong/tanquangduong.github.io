...
```python
# Same CPU task for all approaches
...
# 1) Threading (not ideal for CPU-bound code in CPython because of GIL)
thread_results = [None] * 4

def thread_worker(index: int, n: int) -> None:
    thread_results[index] = cpu_heavy(n)

threads = [threading.Thread(target=thread_worker, args=(i, 10**7)) for i in range(4)]
for t in threads:
    t.start()
for t in threads:
    t.join()

# 2) Multiprocessing (good for CPU-bound work: true parallelism)
with Pool(4) as pool:
    process_results = pool.map(cpu_heavy, [10**7] * 4)

# 3) asyncio (good for I/O-bound work, not CPU-bound loops)
async def fake_io_call(delay: float) -> str:
    await asyncio.sleep(delay)  # Simulate network/file/database wait
    return f"done in {delay}s"

async def run_asyncio_demo() -> list[str]:
    return await asyncio.gather(fake_io_call(1.0), fake_io_call(1.2), fake_io_call(0.8))

asyncio_results = asyncio.run(run_asyncio_demo())
```
