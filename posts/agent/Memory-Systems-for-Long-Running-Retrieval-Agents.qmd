...
- Build the agent architectures that use these memory systems with [Building Agents with LangGraph](/posts/agent/Building-Agents-with-LangGraph.html) — covers StateGraph, checkpointers, human-in-the-loop, and subgraphs.
- Coordinate multiple memory-enabled agents with [Multi-Agent RAG Orchestration Patterns](/posts/agent/Multi-Agent-RAG-Orchestration-Patterns.html) — supervisor, swarm, and hierarchical topologies.
- Understand the reasoning loop that drives tool selection with [Building a ReAct Agent from Scratch](/posts/agent/Building-a-ReAct-Agent-from-Scratch.html) — the Thought-Action-Observation cycle.
- Wire agents to retrieval tools with [Tool Use and Function Calling for Retrieval Agents](/posts/agent/Tool-Use-and-Function-Calling-for-Retrieval-Agents.html) — from function calling to MCP.
- Build the retrieval pipelines these agents query with [Building a RAG Pipeline from Scratch](/posts/rag/Building-a-RAG-Pipeline-from-Scratch.html).
- Add self-correcting retrieval with [Hybrid and Corrective RAG Architectures](/posts/rag/Hybrid-and-Corrective-RAG-Architectures.html).
- Scale memory-backed agents in production with [RAG in Production: Scaling, Caching, and Observability](/posts/rag/RAG-in-Production-Scaling-Caching-and-Observability.html).
- Monitor multi-turn agent behavior with [Observability for Multi-Turn LLM Conversations](/posts/llm/Observability-for-Multi-Turn-LLM-Conversations.html).
