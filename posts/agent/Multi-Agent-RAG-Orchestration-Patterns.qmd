...
- Build the individual agents these topologies compose with [Building a ReAct Agent from Scratch](/posts/agent/Building-a-ReAct-Agent-from-Scratch.html) — covers the Thought-Action-Observation cycle that powers each worker.
- Wire agents together using graph primitives with [Building Agents with LangGraph](/posts/agent/Building-Agents-with-LangGraph.html) — state machines, conditional routing, checkpointing, and human-in-the-loop.
- Connect agents to external systems with [Tool Use and Function Calling for Retrieval Agents](/posts/agent/Tool-Use-and-Function-Calling-for-Retrieval-Agents.html) — from OpenAI function calling to MCP.
- Build the retrieval pipelines agents call with [Building a RAG Pipeline from Scratch](/posts/rag/Building-a-RAG-Pipeline-from-Scratch.html) — chunking, embedding, retrieval, and generation.
- Add self-correcting retrieval with [Hybrid and Corrective RAG Architectures](/posts/rag/Hybrid-and-Corrective-RAG-Architectures.html) — the corrective patterns used in the multi-source RAG workflow.
- Route agent queries to the right retrieval pipeline with [Agentic RAG: When Retrieval Needs Reasoning](/posts/rag/Agentic-RAG-When-Retrieval-Needs-Reasoning.html).
- Monitor multi-agent behavior in production with [Observability for Multi-Turn LLM Conversations](/posts/llm/Observability-for-Multi-Turn-LLM-Conversations.html).
- Add safety guardrails to agent outputs with [Guardrails for LLM Applications with Giskard](/posts/llm/Guardrails-for-LLM-Applications-with-Giskard.html).
