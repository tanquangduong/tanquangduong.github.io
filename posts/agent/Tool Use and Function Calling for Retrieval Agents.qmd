...
- Understand the ReAct reasoning loop that powers tool selection in [Building a ReAct Agent from Scratch](/posts/agent/Building%20a%20ReAct%20Agent%20from%20Scratch.html).
- Connect vector search, SQL, and API tools to RAG pipelines with [Building a RAG Pipeline from Scratch](/posts/rag/Building%20a%20RAG%20Pipeline%20from%20Scratch.html).
- Add safety validation to tool inputs and agent outputs with [Guardrails for LLM Applications](/posts/llm/Guardrails%20for%20LLM%20Applications%20with%20Giskard.html).
- Monitor tool calls and agent behavior in production with [Observability for Multi-Turn LLM Conversations](/posts/llm/Observability%20for%20Multi-Turn%20LLM%20Conversations.html).
- See how agents route queries across multiple retrieval sources in [Agentic RAG: When Retrieval Needs Reasoning](/posts/rag/Agentic%20RAG%20When%20Retrieval%20Needs%20Reasoning.html).
