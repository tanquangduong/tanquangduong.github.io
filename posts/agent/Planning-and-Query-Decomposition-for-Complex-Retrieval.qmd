...
description: "Plan-and-execute agents, sub-question generation, and multi-hop retrieval over heterogeneous data sources"
...
![Query decomposition and multi-hop retrieval planning architecture](/images/agent/planning-query-decomposition-retrieval-300px.png){fig-align="center" width=300}
...
- Implement the agent loop that executes each plan step with [Building a ReAct Agent from Scratch](/posts/agent/Building-a-ReAct-Agent-from-Scratch.html) — the Thought-Action-Observation cycle used inside each executor.
- Build the state machines that orchestrate plan-and-execute flows with [Building Agents with LangGraph](/posts/agent/Building-Agents-with-LangGraph.html) — StateGraph, conditional routing, checkpointers, and subgraphs.
- Distribute sub-questions across specialized retrieval agents with [Multi-Agent RAG Orchestration Patterns](/posts/agent/Multi-Agent-RAG-Orchestration-Patterns.html) — supervisor and hierarchical topologies.
- Give planning agents persistent memory across sessions with [Memory Systems for Long-Running Retrieval Agents](/posts/agent/Memory-Systems-for-Long-Running-Retrieval-Agents.html) — scratchpads, episodic recall, and cross-agent sharing.
- Connect plan steps to retrieval tools with [Tool Use and Function Calling for Retrieval Agents](/posts/agent/Tool-Use-and-Function-Calling-for-Retrieval-Agents.html) — function calling, MCP, and dynamic tool selection.
- Build the retrieval pipelines sub-questions query with [Building a RAG Pipeline from Scratch](/posts/rag/Building-a-RAG-Pipeline-from-Scratch.html).
- Add self-correcting retrieval within plan steps with [Hybrid and Corrective RAG Architectures](/posts/rag/Hybrid-and-Corrective-RAG-Architectures.html).
- Route sub-questions to knowledge graphs with [GraphRAG: Knowledge Graphs Meet Retrieval-Augmented Generation](/posts/rag/GraphRAG-Knowledge-Graphs-Meet-Retrieval-Augmented-Generation.html).
- Monitor multi-step agent execution with [Observability for Multi-Turn LLM Conversations](/posts/llm/Observability-for-Multi-Turn-LLM-Conversations.html).
