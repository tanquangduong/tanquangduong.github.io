...
Autonomous retrieval agents are powerful — they can [plan multi-step queries](/posts/agent/Planning%20and%20Query%20Decomposition%20for%20Complex%20Retrieval.html), [call tools](/posts/agent/Tool%20Use%20and%20Function%20Calling%20for%20Retrieval%20Agents.html), [orchestrate sub-agents](/posts/agent/Multi-Agent%20RAG%20Orchestration%20Patterns.html), and [conduct deep research](/posts/agent/Deep%20Research%20Agents%20from%20RAG%20to%20Autonomous%20Investigation.html). But every capability you grant an agent is a capability an attacker can exploit.
...
The [ReAct loop](/posts/agent/Building%20a%20ReAct%20Agent%20from%20Scratch.html) gives agents the power to call tools — and that is exactly where **excessive agency** (OWASP LLM06) manifests. Authorization gates ensure the agent can only call approved tools with approved arguments.
...
Integrate the authorization gate into a [LangGraph](/posts/agent/Building%20Agents%20with%20LangGraph.html) state graph using the `interrupt` pattern for human-in-the-loop confirmation on sensitive actions:
...
**Unbounded consumption** (OWASP LLM10) is a real production risk. A [deep research agent](/posts/agent/Deep%20Research%20Agents%20from%20RAG%20to%20Autonomous%20Investigation.html) might legitimately need dozens of tool calls — but an exploited agent could make thousands.
...
No single guardrail stops every attack. Prompt injection in particular remains an open research problem. The key insight is **defense-in-depth**: each layer independently limits damage, and together they make exploitation dramatically harder. Combined with [human-in-the-loop checkpoints](/posts/agent/Building%20Agents%20with%20LangGraph.html), [observability](/posts/llm/Observability%20for%20Multi-Turn%20LLM%20Conversations.html), and structured monitoring, you get a production agent system that is both capable and trustworthy.
...
- Lock down the [ReAct agent loop](/posts/agent/Building%20a%20ReAct%20Agent%20from%20Scratch.html) that these guardrails protect — understand the Thought-Action-Observation cycle before adding safety layers.
- Apply authorization gates to [tool calls and function calling](/posts/agent/Tool%20Use%20and%20Function%20Calling%20for%20Retrieval%20Agents.html) — see how agents select and invoke tools in practice.
- Integrate guardrails into [LangGraph state machines](/posts/agent/Building%20Agents%20with%20LangGraph.html) — use `interrupt`, checkpointers, and conditional routing for human-in-the-loop approval.
- Add policy enforcement across [multi-agent orchestration patterns](/posts/agent/Multi-Agent%20RAG%20Orchestration%20Patterns.html) — supervisor agents can enforce global budgets and tool permissions.
- Protect [long-running agent memory](/posts/agent/Memory%20Systems%20for%20Long-Running%20Retrieval%20Agents.html) from poisoning — memory injection is an emerging attack vector for persistent agents.
- Apply budget limits to [planning and query decomposition](/posts/agent/Planning%20and%20Query%20Decomposition%20for%20Complex%20Retrieval.html) — multi-step plans can amplify token consumption.
- Secure [deep research agents](/posts/agent/Deep%20Research%20Agents%20from%20RAG%20to%20Autonomous%20Investigation.html) that make dozens of web searches — budget and sandbox controls are critical for open-ended investigation.
- Explore [Guardrails for LLM Applications with Giskard](/posts/llm/Guardrails%20for%20LLM%20Applications%20with%20Giskard.html) — a complementary approach using Giskard for vulnerability scanning and model testing.
- Track agent behavior across sessions with [Observability for Multi-Turn LLM Conversations](/posts/llm/Observability%20for%20Multi-Turn%20LLM%20Conversations.html).
